...
{
    class RightBottomDiagonalFinisher : IMoveFinisher
    {
        ...
        public RightBottomDiagonalFinisher(List<Cell> cells, GameField field)
        {
            _cells = cells;
            _field = field;
        }
        ...
        public void DeactivateFinishedCells()
        {
            ...
            {
                if (cell.CellRectangle == _field.FieldCells[cell.Coordinates.X, cell.Coordinates.Y, 2].CellRectangle)
                {
                    cell.Active = false;
                    cell.Coordinates = new GameCoordinates(cell.Coordinates.X, cell.Coordinates.Y, 2);
                    _field.FieldCells[cell.Coordinates.X, cell.Coordinates.Y, cell.Coordinates.Z].IsEmpty = false;
                    continue;
                }

                if (cell.Coordinates.Z < 2)
                {
        ...
        public void ActivateFalselyFinishedCells()
        {
            foreach (var cell in _cells)
            {
                if (cell.Coordinates.Z < 2 && !cell.Active)
                {
                    if (_field.FieldCells[cell.Coordinates.X, cell.Coordinates.Y, cell.Coordinates.Z + 1].IsEmpty)
                    {
                        cell.Active = true;
